...
const DashCreateCategory = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, actions) => {
      axios
        .post('http://localhost:8000/api/type/create', values)
        .then((response) => {
          // Handle success if needed
          console.log(response.data);
          // Dispatch an action to update the category list after creation
          dispatch(createJobTypeAction(response.data.jobType));
          actions.resetForm();
        })
        .catch((error) => {
