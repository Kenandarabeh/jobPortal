...
const DashCategory = () => {
    ...
    const deleteJobById = (e, id) => {
        axios
            .delete(`http://localhost:8000/api/type/delete/${id}`)
            .then((response) => {
                // Handle success if needed
                console.log(response.data);
                // Dispatch an action to update the job list after deletion
                dispatch(jobTypeLoadAction());
            })
            .catch((error) => {
    ...
    return (
        <Box >
            ...
            <Box sx={{ pb: 2, display: "flex", justifyContent: "right" }}>
                <Button variant="contained" color="success" startIcon={<AddIcon />}><Link style={{ color: "white"}} to='/admin/category/create'>Create category</Link></ Button>
            </Box>
